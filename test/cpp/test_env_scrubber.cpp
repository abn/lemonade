...
int main() {
    TestResult r;
    std::printf("=== EnvScrubber Secret Stripping Unit Tests ===\n\n");

    // 1. LEMONADE_* Secret Variables Identification
    {
    ...
    // 3. Infrastructure & Repository Tokens
    {
    ...
    // 4. Safe Runtime Allowlists Preservation
    {
        ...
        r.check(EnvScrubber::is_allowlisted_key("CUDA_VISIBLE_DEVICES"), "CUDA_VISIBLE_DEVICES is allowlisted");
        r.check(EnvScrubber::is_allowlisted_key("ESPEAK_DATA_PATH"), "ESPEAK_DATA_PATH is allowlisted");
        r.check(!EnvScrubber::is_allowlisted_key("RANDOM_CUSTOM_VAR"), "RANDOM_CUSTOM_VAR is not allowlisted");
    }

    // 5. Environment Sanitization (Isolated custom_env only)
    {
        ...
        auto sanitized = EnvScrubber::sanitize_environment(mixed_env, {}, /*include_ambient=*/false);

        r.check(sanitized.size() == 4, "sanitized environment stripped all 4 secret variables");
        ...
        r.check(has_py, "PYTHONNOUSERSITE preserved with original value");
        r.check(has_espeak, "ESPEAK_DATA_PATH preserved with original value");
    ...
    // 7. Ambient Environment Sanitization
    {
        auto ambient_sanitized = EnvScrubber::sanitize_environment({}, {}, /*include_ambient=*/true);
