...
public:
    ...
    static std::unique_ptr<SandboxEngine> create_linux_landlock_engine();
    static std::unique_ptr<SandboxEngine> create_macos_seatbelt_engine();
    static std::unique_ptr<SandboxEngine> create_windows_fallback_engine();
    ...
    // macOS Seatbelt SBPL helpers (testable cross-platform)
    static std::string generate_seatbelt_profile(const SandboxPolicy& policy);
    static std::string escape_sbpl_string(const std::string& input);

    // Linux Landlock helpers (testable cross-platform)
    static int get_landlock_abi_version();
    static uint64_t compute_landlock_fs_mask(int abi);
    static uint64_t compute_landlock_net_mask(int abi);
};
