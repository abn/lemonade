...
bool EnvScrubber::is_sensitive_key(const std::string& key) {
    ...
    const auto& exact = sensitive_exact_keys();
    if (exact.find(upper) != exact.end()) {
        return true;
    }
...
bool EnvScrubber::is_allowlisted_key(const std::string& key) {
    ...
    const std::string upper = to_upper_ascii(key);
    const auto& allowlist = default_allowlist();
    return allowlist.find(upper) != allowlist.end();
}
